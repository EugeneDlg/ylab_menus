...
from app.cache import get_cache_session, Cache
...
class Service:
    ...
    def add_menu(self, menu: dict):
        ...
        Cache(self.cache).delete_cache("list::")
        return menu

    def get_menu(self, menu_id: int):
        menu = Cache(self.cache).get_cache(f"{menu_id}::")
        if menu is None:
            menu_t = Database(self.session).get_menu(menu_id)
            if menu_t is not None:
                menu = add_counts_to_menu(menu_t)
                Cache(self.cache).set_cache(f"{menu_id}::", menu)
        return menu

    def get_menu_list(self):
        menu_list = Cache(self.cache).get_cache("list::")
    ...
    def delete_menu(self, menu_id: int):
        ...
        Cache(self.cache).delete_cache(f"{menu_id}:", True)
        return response
    ...
    def add_submenu(self, menu_id: int, submenu: dict):
        ...
        Cache(self.cache).delete_cache("list::")
        Cache(self.cache).delete_cache(f"{menu_id}::")
        Cache(self.cache).delete_cache(f"{menu_id}:list:")
        return menu

    def get_submenu(self, menu_id: int, submenu_id: int):
        submenu = Cache(self.cache).get_cache(f"{menu_id}:{submenu_id}:")
        if submenu is None:
            ...
            if submenu_t is not None:
                ...
                Cache(self.cache).set_cache(f"{menu_id}:{submenu_id}:", submenu)
        return submenu

    def get_submenu_list(self, menu_id: int):
        submenu_list = Cache(self.cache).get_cache(f"{menu_id}:list:")
        if submenu_list is None:
            ...
            Cache(self.cache).set_cache(f"{menu_id}:list:", submenu_list)
        return submenu_list
    ...
    def update_submenu(self, menu_id: int, submenu_id: int, submenu: dict):
        ...
        # delete_cache(f"list::")
        Cache(self.cache).delete_cache(f"{menu_id}:list:")
        Cache(self.cache).delete_cache(f"{menu_id}:{submenu_id}:")
        return submenu
    ...
    def delete_submenu(self, menu_id: int, submenu_id: int):
        ...
        Cache(self.cache).delete_cache(f"{menu_id}::")
        Cache(self.cache).delete_cache("list::")
        Cache(self.cache).delete_cache(f"{menu_id}:{submenu_id}:", True)
        Cache(self.cache).delete_cache(f"{menu_id}:list:")
        return response
    ...
    def add_dish(self, menu_id: int, submenu_id: int, dish: dict):
        ...
        Cache(self.cache).delete_cache(f"{menu_id}:list:")
        Cache(self.cache).delete_cache(f"{menu_id}:{submenu_id}:")
        Cache(self.cache).delete_cache(f"{menu_id}:{submenu_id}:list")
        return dish

    def get_dish(self, menu_id: int, submenu_id: int, dish_id: int):
        dish = Cache(self.cache).get_cache(f"{menu_id}:{submenu_id}:{dish_id}")
        if dish is None:
            dish = Database(self.session).get_dish(menu_id, submenu_id, dish_id)
            Cache(self.cache).set_cache(f"{menu_id}:{submenu_id}:{dish_id}", dish)
        return dish

    def get_dish_list(self, menu_id, submenu_id):
        dish_list = Cache(self.cache).get_cache(f"{menu_id}:{submenu_id}:list")
        if dish_list is None:
            dish_list = Database(self.session).get_dish_list(menu_id, submenu_id)
            Cache(self.cache).set_cache(f"{menu_id}:{submenu_id}:list:", dish_list)
        return dish_list
    ...
    def update_dish(self, menu_id: int, submenu_id, dish_id: int, dish: dict):
        ...
        Cache(self.cache).delete_cache(f"{menu_id}:{submenu_id}:list")
        Cache(self.cache).delete_cache(f"{menu_id}:{submenu_id}:{dish_id}")
        return dish
...
def get_service(
    ...
    return Service(session, cache_session)
