...
from db_models import (
    Sessions, Menu, Submenu, Dish,
    create_tables, delete_tables, get_menu_item,
...
@app.post("/api/v1/menus",
          response_model=ResponseMenuModel, status_code=status.HTTP_201_CREATED)
...
@app.get("/api/v1/menus/{menu_id}",
         response_model=ResponseMenuModel, status_code=status.HTTP_200_OK)
...
@app.get("/api/v1/menus",
         response_model=List[ResponseMenuModel], status_code=status.HTTP_200_OK)
...
@app.patch("/api/v1/menus/{menu_id}",
           response_model=ResponseMenuModel, status_code=status.HTTP_200_OK)
def update_menu(menu_id: int, menu: UpdateMenuModel, db: Session = Depends(get_session)):
    menu_t = get_menu_item(menu_id, db)
    if is_found(menu_t, "menu"):
        menu_ = menu_t[0]
...
@app.get("/api/v1/menus/{menu_id}/submenus/{submenu_id}",
         response_model=ResponseSubmenuModel, status_code=status.HTTP_200_OK)
...
@app.patch("/api/v1/menus/{menu_id}/submenus/{submenu_id}",
           response_model=ResponseSubmenuModel, status_code=status.HTTP_200_OK)
def update_submenu(menu_id: int, submenu_id: int, submenu: UpdateSubmenuModel, db: Session = Depends(get_session)):
    submenu_t = get_submenu_item(menu_id, submenu_id, db)
    if is_found(submenu_t, "submenu"):
...
@app.post("/api/v1/menus/{menu_id}/submenus/{submenu_id}/dishes",
          response_model=ResponseDishModel, status_code=status.HTTP_201_CREATED)
def add_dish(menu_id: int, submenu_id: int, dish: DishModel, db: Session = Depends(get_session)):
    submenu_t = get_submenu_item(menu_id, submenu_id, db)
    if is_found(submenu_t, "submenu"):
        submenu = submenu_t[0]
        dish = Dish(**dish.dict())
...
@app.get("/api/v1/menus/{menu_id}/submenus/{submenu_id}/dishes/{dish_id}",
         response_model=ResponseDishModel, status_code=status.HTTP_200_OK)
def get_dish(menu_id: int, submenu_id: int, dish_id: int, db: Session = Depends(get_session)):
...
def get_dishes(menu_id: int, submenu_id: int, db: Session = Depends(get_session)):
